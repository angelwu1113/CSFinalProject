...
class Board:
    ...
    def gravity_backToRight(self):
        '''
        pushing all the piece to the right
        '''
    ...
    def gravity_backToLeft(self):
        '''
        pushing all the piece to the left
        '''
        list = self.gravity_storeData()
        new_board= [[' ']*self.width for row in range(self.height)]
        for i in range(len(list)):
    ...
    def aiMove(self, ox):
        ...
        for i in range(self.width):
            ...
            if self.allowsMove(i) == True:
                return i
        '''
    def colsToMake3(self, ox):
        """
        Return columns where ox can create a 3-in-a-row by playing there.
        """
        result = []
        for col in range(self.width):
            if not self.allowsMove(col):
                continue
            self.addMove(col, ox)
            # check if this creates any 3-in-a-row
            if self.count_streak(ox, 3) > 0:
                made3 = True
            else:
                made3 = False
            self.delMove(col)
    ...
    def colsToMake2(self, ox):
        """
        Return columns where ox can create a 2-in-a-row by playing there.
        """
        result = []
        for col in range(self.width):
            if not self.allowsMove(col):
    ...
    def count_streak(self, ox, N):
        """
        Count number of N-in-a-row opportunities (not necessarily complete, but open-ended).
        """
        count = 0
        H = self.height
        W = self.width
        ...
        for r in range(H):
            for c in range(W):
                # horizontal
                if inarow_Neast(ox, r, c, D, N):
                    count += 1
                # vertical
                if inarow_Nsouth(ox, r, c, D, N):
                    count += 1
                # diag /
                if inarow_Nnortheast(ox, r, c, D, N):
                    count += 1
                # diag \
                if inarow_Nsoutheast(ox, r, c, D, N):
                    count += 1
        return count
    
    '''
    def evaluateBoard(self, ox):
        
        getting the best next moves through scoring
        ...
        if self.winsFor(opponent):
            return 9000
        
        # Heuristics
        score += 50 * self.count_streak(ox, 2)
        score += 200 * self.count_streak(ox, 3)

        # Penalize letting opponent get immediate win
        if self.colsToWin(opponent) != []:
            score -= 200
    ...
    def aivsaiMove(self, ox):
        '''
        create a more interlectual ai's rmovement
        '''
        myWin = self.colsToWin(ox)
        if myWin:
            return myWin[0]
        ...
        blockWin = self.colsToWin(opponent)
        if blockWin:
            return blockWin[0]

        # block opponent's 3-in-a-row attempts
        block3 = self.colsToMake3(opponent)
        if block3:
            return block3[0]

        # block opponent's 2-in-a-row attempts
        block2 = self.colsToMake2(opponent)
        if block2:
            return block2[0]

        # create my own 3-in-a-row
        make3 = self.colsToMake3(ox)
        if make3:
            return make3[0]

        # create my own 2-in-a-row
        make2 = self.colsToMake2(ox)
        if make2:
            return make2[0]
    ...
    def aihostGame(self):
        ...
        while True:
            ...
            if self.winsFor('X') == True:
                print(self)
                print("X wins! Thanks for watching.")
                break
            ...
            if self.winsFor('O') == True:
                print(self)
                print("O wins! Thanks for watching.")
                break

            if self.isFull() == True:
                print(self)
                print("The game draws! Thanks for watching. :D")
                break
...
userchoice = input("Welcome. Do you want to play game with AI or watch AI vs AI?  play/watch")
b = Board(7, 6)
if userchoice == 'play':
    b.hostGame()
else:
